...
def _build_stats(data: DrawingData, model: Model3D, wall_height: float) -> dict:
    ...
    # yalnızca duvar (yoksa diğer eleman) parçalarının sınırlarından hesapla.
    segs = data.wall_segments or (
        data.door_segments + data.window_segments + data.column_segments)
    if segs:
        xs = [c for s in segs for c in (s.x0, s.x1)]
        ys = [c for s in segs for c in (s.y0, s.y1)]
        width_m, depth_m = max(xs) - min(xs), max(ys) - min(ys)
    else:
        width_m, depth_m = data.width_m, data.depth_m
    return {
        "Birim": data.unit_name,
        "Genişlik (m)": f"{width_m:.2f}",
        "Derinlik (m)": f"{depth_m:.2f}",
        "Taban Alanı (m²)": f"{max(0.0, width_m) * max(0.0, depth_m):.1f}",
